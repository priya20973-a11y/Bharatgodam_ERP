    ...
    <form onSubmit={handleSubmit} className="space-y-4 p-6 border rounded-lg bg-slate-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {autoGradingType === 'Grading' && (
          <div className="space-y-2">
            <label className="text-sm font-medium">{t('inward.grade')}</label>
            <Select value={grade} onValueChange={setGrade} required>
              <SelectTrigger><SelectValue placeholder={t('inward.selectGrade')} /></SelectTrigger>
              <SelectContent>
                <SelectItem value="Large">{t('inward.gradeLarge')}</SelectItem>
                <SelectItem value="Small">{t('inward.gradeSmall')}</SelectItem>
                <SelectItem value="Mixed">{t('inward.gradeMixed')}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        )}
