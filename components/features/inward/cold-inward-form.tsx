  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < clientSections.length; i++) {
      ...
      for (const s of c.stacks) {
        ...
        grandTotalAllocated += Number(s.allocatedWeight) || 0;
      }
      grandTotalBags += Number(c.bagsCount || 0) + Number(c.jin || 0) + Number(c.mixed || 0);
    }

    if (Math.abs(grandTotalAllocated - commonNetWeight) > 0.01) {
      toast.error(`Total allocated weight across all clients (${grandTotalAllocated}) does not match the Common Net Weight (${commonNetWeight})`);
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="p-6 border rounded-lg bg-slate-50 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 pt-4 border-t bg-slate-100 p-4 rounded-md">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-green-700">Net Weight (Auto)</label>
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium">Kata Bharati (Auto)</label>
            <div className="px-3 py-2 border rounded-md bg-white font-bold text-slate-700">
              {(() => {
      ...
      {clientSections.map((client, cIdx) => {
        ...
        return (
          <div key={client.id} className="p-6 border rounded-lg bg-white shadow-sm space-y-4">
            ...
            <div className={`grid grid-cols-1 ${!common.sameCommodity && autoGradingType === 'Grading' ? 'md:grid-cols-2' : 'md:grid-cols-1'} gap-4 bg-slate-50 p-4 rounded-md`}>
              {!common.sameCommodity && (
                <div className="space-y-2">
                  <label className="text-sm font-medium">{t('inward.commodityVariety')} *</label>
              ...
              {common.sameCommodity && autoGradingType !== 'Grading' && (
                <div className="text-sm text-slate-500 italic flex items-center h-full pt-6">
