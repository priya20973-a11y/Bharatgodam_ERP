...
interface StackQRScannerModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function StackQRScannerModal({
  isOpen,
  onClose,
  onScanSuccess,
  initialTab = 'camera',
}: StackQRScannerModalProps) {
  ...
  useEffect(() => {
    ...
    const startNativeCamera = async () => {
      // Check native browser mediaDevices support
      ...
      // 1. Prefer rear/environment camera on mobile
      try {
        ...
        try {
          // 2. Fallback to default webcam (laptop/desktop)
          stream = await navigator.mediaDevices.getUserMedia({
            video: true,
            audio: false,
          });
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Try Html5Qrcode scanFile first
      let decodedText: string | null = null;
      try {
        const html5QrCode = new Html5Qrcode('stack-qr-file-temp');
        decodedText = await html5QrCode.scanFile(file, false);
        html5QrCode.clear();
      } catch (err) {
        // Fallback to image tag + canvas + jsQR decoding for image files
        const img = new Image();
      ...
      if (decodedText) {
        ...
        toast.error('Could not detect a valid QR code in the uploaded image. Please upload a clear image of the Stack QR.');
      }
    } catch (err: any) {
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-indigo-900">
            ...
            Stack QR Code Reader
          </DialogTitle>
        </DialogHeader>

        {/* Hidden elements for canvas frame decoding & file scanner */}
        <canvas ref={canvasRef} className="hidden" />
        <div id="stack-qr-file-temp" className="hidden" />
              ...
              {cameraError ? (
                ...
                <div className="relative w-full h-[260px] flex items-center justify-center bg-black">
                  ...
                  <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                    <div className="w-48 h-48 border-2 border-indigo-400 rounded-lg shadow-[0_0_0_9999px_rgba(0,0,0,0.4)] relative">
            ...
            <form onSubmit={handleManualSubmit} className="space-y-2 border-t pt-3">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  placeholder="Scan or paste QR code text..."
        ...
        {activeTab === 'upload' && (
          <div className="space-y-4 pt-1">
            <div
              ...
              className="border-2 border-dashed border-indigo-300 bg-indigo-50/50 hover:bg-indigo-50 p-6 rounded-lg text-center cursor-pointer transition-all flex flex-col items-center justify-center space-y-2"
            >
              <input
                ref={fileInputRef}
                type="file"
                ...
                <div className="space-y-2">
                  ...
                  {processingFile && (
                    <div className="flex items-center justify-center gap-2 text-xs font-semibold text-indigo-700">
                      <Loader2 className="w-4 h-4 animate-spin" /> Decoding QR image...
                    </div>
                  )}
                </div>
              ) : (
                <>
                  <div className="p-3 bg-white rounded-full border border-indigo-200 text-indigo-600 shadow-xs">
                    <ImageIcon className="w-6 h-6" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-indigo-900">Click to Select QR Code Image</p>
                    <p className="text-xs text-slate-500 mt-1">Supports PNG, JPG, JPEG, WEBP files</p>
                  </div>
