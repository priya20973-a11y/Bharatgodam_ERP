...
export default function ColdTransferForm({ clients }: ColdTransferFormProps) {
  ...
  useEffect(() => {
    async function fetchInwards() {
      ...
      try {
        const data = await getAvailableInwardsForTransfer(fromClientId);
    ...
    fetchInwards();
    setValue('inwardId', ''); // reset selected inward when client changes
  }, [fromClientId, setValue]);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <SelectContent>
                      {availableInwards.map(inv => (
                        <SelectItem key={inv._id} value={inv._id}>
                          {inv.date ? new Date(inv.date).toISOString().slice(0, 10) : ''} - {inv.commodityId?.name} ({formatNumber(inv.availableQty)} kg)
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
          ...
          <div className="bg-slate-50 p-4 rounded-md border text-sm space-y-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <p className="font-bold text-red-600">{formatNumber(selectedInwardDetails.availableQty)} kg</p>
              </div>
              <div>
                ...
                <div className="mt-1 space-y-1">
                  {selectedInwardDetails.availableAllocations.map((alloc: any, idx: number) => (
                    <div key={idx} className="bg-white border px-3 py-1.5 rounded-sm flex justify-between">
                      ...
                      <span className="font-medium">{formatNumber(alloc.allocatedWeight)} kg ({alloc.bagsCount} bags)</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="space-y-4 pt-4 border-t">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            <div className="space-y-2 flex flex-col">
              <label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Transfer Net Weight (kg) *</label>
              <Controller
                name="transferWeight"
                control={control}
                render={({ field }) => (
