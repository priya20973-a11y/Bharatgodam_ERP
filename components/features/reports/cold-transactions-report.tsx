...
export default function ColdTransactionsReport({ transactions, isAdmin = false }: ColdTransactionsReportProps) {
  ...
  const columns = useMemo<ColumnDef<TransactionRecord>[]>(() => [
    ...
    },
    {
      ...
      header: 'Stock Type',
      cell: ({ row }) => {
        const type = row.getValue('stockType');
        return <span>{type ? String(type) : '—'}</span>;
      },
    },
    {
      accessorKey: 'purchaseQuantityKg',
      header: 'Purchase Qty',
      cell: ({ row }) => {
        const value = row.getValue('purchaseQuantityKg');
        if (typeof value !== 'number') return <span className="text-slate-400">—</span>;
        return <span>{value.toFixed(2)}</span>;
      },
    },
    {
      accessorKey: 'selfQuantityKg',
      header: 'Self Qty',
      cell: ({ row }) => {
        const value = row.getValue('selfQuantityKg');
        if (typeof value !== 'number') return <span className="text-slate-400">—</span>;
        return <span>{value.toFixed(2)}</span>;
      },
    },
