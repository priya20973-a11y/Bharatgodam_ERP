...
export function calculateRentMonths(inwardDate: Date, outwardDate: Date): number {
  if (!inwardDate || !outwardDate) return 1;
  const inD = new Date(inwardDate);
  const outD = new Date(outwardDate);
  if (isNaN(inD.getTime()) || isNaN(outD.getTime())) return 1;

  const d1 = inD < outD ? inD : outD;
  const d2 = inD < outD ? outD : inD;
