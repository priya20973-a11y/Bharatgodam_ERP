...
function renderTable(rows: TableRow[], columns: string[]) {
      ...
      <table className="min-w-full divide-y divide-slate-200 text-sm">
        ...
        <tbody className="divide-y divide-slate-200 bg-white">
          {rows.map((row, index) => {
            ...
            const rowKey = typeof idValue === 'string' ? idValue : String(index);
            return (
...
export default async function ColdAccountingSectionPage({ params }: { params: Promise<{ section: string }> }) {
    ...
    return (
      <div className="min-h-screen bg-slate-50 p-6">
        <div className="mx-auto max-w-6xl space-y-6">
          <h1 className="text-2xl font-bold text-slate-900">{title}</h1>
          {renderTable(accounts.map((account: TableRow) => ({ _id: account.id, code: account.code, name: account.name, type: account.type, parent: account.parent })), ['code', 'name', 'type', 'parent'])}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 p-6">
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          {renderTable(journals.map((entry: TableRow) => ({ _id: String(entry._id ?? ''), entryDate: entry.entryDate, voucherNumber: entry.voucherNumber, narration: entry.narration, totalDebit: entry.totalDebit, totalCredit: entry.totalCredit, createdBy: entry.createdBy })), ['entryDate', 'voucherNumber', 'narration', 'totalDebit', 'totalCredit', 'createdBy'])}
        </div>
      </div>
    );
  }
  ...
  if (section === 'ledger') {
    ...
    return (
      <div className="min-h-screen bg-slate-50 p-6">
        <div className="mx-auto max-w-7xl space-y-6">
          <h1 className="text-2xl font-bold text-slate-900">{title}</h1>
          {renderTable(ledger.map((row: TableRow) => ({ accountName: row.accountName, accountType: row.accountType, openingBalance: row.openingBalance, debitTotal: row.debitTotal, creditTotal: row.creditTotal, closingBalance: row.closingBalance })), ['accountName', 'accountType', 'openingBalance', 'debitTotal', 'creditTotal', 'closingBalance'])}
        </div>
      </div>
    );
  }
  ...
  if (section === 'trial-balance') {
    ...
    return (
      <div className="min-h-screen bg-slate-50 p-6">
        <div className="mx-auto max-w-7xl space-y-6">
          <h1 className="text-2xl font-bold text-slate-900">{title}</h1>
          {renderTable(trial.rows.map((row: TableRow) => ({ accountName: row.accountName, accountType: row.accountType, openingBalance: row.openingBalance, debit: row.debit, credit: row.credit, closingBalance: row.closingBalance })), ['accountName', 'accountType', 'openingBalance', 'debit', 'credit', 'closingBalance'])}
  ...
  if (section === 'cash-book') {
    ...
    return (<div className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-7xl space-y-6"><h1 className="text-2xl font-bold text-slate-900">{title}</h1>{renderTable(cash.map((row: TableRow) => {
      const lines = row.lines as Record<string, unknown> | undefined;
      return { entryDate: row.entryDate, voucherNumber: row.voucherNumber, narration: row.narration, debit: lines?.debit, credit: lines?.credit };
    }), ['entryDate', 'voucherNumber', 'narration', 'debit', 'credit'])}</div></div>);
  }

  if (section === 'bank-book') {
    const bank = await getAccountingBankBook();
    return (<div className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-7xl space-y-6"><h1 className="text-2xl font-bold text-slate-900">{title}</h1>{renderTable(bank.map((row: TableRow) => {
      const lines = row.lines as Record<string, unknown> | undefined;
      return { entryDate: row.entryDate, voucherNumber: row.voucherNumber, narration: row.narration, debit: lines?.debit, credit: lines?.credit };
    }), ['entryDate', 'voucherNumber', 'narration', 'debit', 'credit'])}</div></div>);
  }
  ...
  if (section === 'day-book') {
    ...
    return (<div className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-7xl space-y-6"><h1 className="text-2xl font-bold text-slate-900">{title}</h1>{renderTable(dayBook.map((row: TableRow) => ({ entryDate: row.entryDate, voucherNumber: row.voucherNumber, narration: row.narration, totalDebit: row.totalDebit, totalCredit: row.totalCredit })), ['entryDate', 'voucherNumber', 'narration', 'totalDebit', 'totalCredit'])}</div></div>);
  }

  if (section === 'gst-reports') {
    const gst = await getAccountingGstReports();
    return (<div className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-7xl space-y-6"><h1 className="text-2xl font-bold text-slate-900">{title}</h1>{renderTable(gst.map((row: TableRow) => {
      const lines = row.lines as Record<string, unknown> | undefined;
      return { entryDate: row.entryDate, voucherNumber: row.voucherNumber, narration: row.narration, debit: lines?.debit, credit: lines?.credit };
    }), ['entryDate', 'voucherNumber', 'narration', 'debit', 'credit'])}</div></div>);
  }
  ...
  if (section === 'financial-year') {
    ...
    return (<div className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-7xl space-y-6"><h1 className="text-2xl font-bold text-slate-900">{title}</h1>{renderTable(years.map((year: TableRow) => ({ year: year.year, startDate: year.startDate, endDate: year.endDate, status: year.status, locked: year.locked ? 'Yes' : 'No' })), ['year', 'startDate', 'endDate', 'status', 'locked'])}</div></div>);
  }

  if (section === 'gst-settings' || section === 'bank-accounts' || section === 'accounting-settings' || section === 'payment-voucher' || section === 'receipt-voucher' || section === 'contra-voucher' || section === 'journal-voucher' || section === 'sales-invoice' || section === 'purchase-voucher') {
    const descriptions: Record<string, string> = {
      'gst-settings': 'Configure GSTIN, state, HSN/SAC, tax rates, invoice prefix and reverse charge settings.',
      'bank-accounts': 'Set up and maintain bank account details, opening balance, QR/UPI and cheque prefix.',
      'accounting-settings': 'Configure accounting automation, voucher approvals and system behavior.',
      'payment-voucher': 'Record payments made for expenses, vendor settlements and bill payments.',
      'receipt-voucher': 'Record receipts collected from clients, customers and other parties.',
      'contra-voucher': 'Transfer funds between cash and bank accounts without affecting income or expense.',
      'journal-voucher': 'Record accounting adjustments and non-cash journal entries.',
      'sales-invoice': 'Generate sales invoices from billing and track revenue by client/warehouse.',
      'purchase-voucher': 'Record purchase invoices, vendor bills, GST details and payment terms.',
    };
      ...
      <div className="min-h-screen bg-slate-50 p-6">
        <div className="mx-auto max-w-6xl space-y-6">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
            <p className="text-slate-600">{descriptions[section]}</p>
            <div className="mt-6 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-slate-700">
              This section is ready for the full Tally-style implementation. You can use this page to build voucher entry forms, GST setup screens and banking masters.
            </div>
          </div>
        </div>
      </div>
