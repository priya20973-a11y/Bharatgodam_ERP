        ...
        {selectedTab === 'clients' ? (
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-[minmax(0,1fr)_280px]">
            <div className="space-y-2">
              <label className="block text-sm font-medium text-slate-700">{t('ledger.selectWarehouse')}</label>
              <select
              ...
              >
                <option value="">{t('ledger.chooseWarehouse')}</option>
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-slate-700">{t('ledger.selectMonth')}</label>
              <select
      ...
      {selectedTab === 'warehouse' ? (
        <div className="space-y-6">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                  <p className="text-sm text-slate-500">{t('ledger.warehouseLabel')}</p>
                  <p className="mt-2 text-lg font-semibold text-slate-900">{warehouseReport.warehouseName}</p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                  <p className="text-sm text-slate-500">{t('ledger.clientsWithInventory')}</p>
                  <p className="mt-2 text-lg font-semibold text-slate-900">{filteredSummary.totalClients}</p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                  <p className="text-sm text-slate-500">{t('ledger.netBalanceMT')}</p>
              ...
              <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm">
                <div className="border-b border-slate-200 bg-slate-50 px-6 py-4">
                  <h2 className="text-lg font-semibold text-slate-900">{t('ledger.clientCommodityLedger')}</h2>
                  <p className="text-sm text-slate-600 mt-1">{t('ledger.clientCommodityLedgerDesc')}</p>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-100 border-b border-slate-200">
                      <tr>
                        <th className="px-4 py-3 text-left font-semibold text-slate-700">{t('ledger.client')}</th>
                        <th className="px-4 py-3 text-left font-semibold text-slate-700">{t('ledger.commodity')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.inwardMT')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.outwardMT')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.balanceMT')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.transactions')}</th>
                      </tr>
                    </thead>
                    <tbody>
              ...
              <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm">
                <div className="border-b border-slate-200 bg-slate-50 px-6 py-4">
                  <h2 className="text-lg font-semibold text-slate-900">{t('ledger.clientLedgerDetails')}</h2>
                  <p className="text-sm text-slate-600 mt-1">{t('ledger.clientLedgerDetailsDesc')}</p>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-100 border-b border-slate-200">
                      <tr>
                        <th className="px-4 py-3 text-left font-semibold text-slate-700">{t('ledger.client')}</th>
                        <th className="px-4 py-3 text-left font-semibold text-slate-700">{t('ledger.commodity')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.totalRent')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.currentInventory')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.inwardMT')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.outwardMT')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.balanceMT')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.transactions')}</th>
              ...
              <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm">
                <div className="border-b border-slate-200 bg-slate-50 px-6 py-4">
                  <h2 className="text-lg font-semibold text-slate-900">{t('ledger.commoditySummary')}</h2>
                  <p className="text-sm text-slate-600 mt-1">{t('ledger.commoditySummaryDesc')}</p>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-100 border-b border-slate-200">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.inwardMT')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.outwardMT')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.netMT')}</th>
                        <th className="px-4 py-3 text-right font-semibold text-slate-700">{t('ledger.client')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredCommoditySummaries.map((commodity: any) => (
                        <tr key={commodity.commodityName} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right text-green-700">{formatNumber(commodity.inwardMT.toFixed(2))}</td>
                          <td className="px-4 py-3 text-right text-red-700">{formatNumber(commodity.outwardMT.toFixed(2))}</td>
                          <td className="px-4 py-3 text-right text-slate-900 font-medium">{formatNumber(commodity.balanceMT.toFixed(2))}</td>
                          <td className="px-4 py-3 text-right text-slate-700">{formatNumber(commodity.clientCount)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {!warehouseLoading && !selectedWarehouseId && (
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-6 text-slate-600">
              {t('ledger.selectWarehousePrompt')}
            </div>
          )}
        </div>
      ) : null}
