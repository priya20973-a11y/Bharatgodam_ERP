...
export async function getColdTransactions() {
  ...
  const combined = [
    ...
    return {
      ...
      chamberNo: t.type === 'INWARD' || t.type === 'OWNERSHIP TRANSFER' ? (t.stackAllocations?.[0]?.chamberName || t.stackAllocations?.[0]?.chamberNo || '') : (t.chamberName || t.chamberNo),
      floorNo: (() => {
        const cName = t.type === 'INWARD' || t.type === 'OWNERSHIP TRANSFER' ? (t.stackAllocations?.[0]?.chamberName || t.stackAllocations?.[0]?.chamberNo) : (t.chamberName || t.chamberNo);
        const fNo = t.type === 'INWARD' || t.type === 'OWNERSHIP TRANSFER' ? t.stackAllocations?.[0]?.floorNo : t.floorNo;
        if (!t.warehouseId?.chambers) return fNo ?? '';
        const chamber = t.warehouseId.chambers.find((c: any) => c.name === cName || c.chamberNo === cName);
        if (!chamber) return fNo ?? '';
        const floor = (chamber.floors || []).find((f: any) => f.floorNo === fNo);
        return floor ? floor.name : (fNo ?? '');
      })(),
      stackNo: t.type === 'INWARD' || t.type === 'OWNERSHIP TRANSFER' ? (t.stackAllocations?.[0]?.stackNo || '') : t.stackNo,
...
export async function updateColdTransaction(id: string, type: 'INWARD' | 'OUTWARD', data: any) {
  ...
  try {
    ...
    if (type === 'INWARD') {
      ...
      const client = await connectToDatabase().then(() => mongoose.model('Client').findById(inward.clientId).lean());
