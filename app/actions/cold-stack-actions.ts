...
export async function getStackDetails(warehouseId: string, chamberName: string, floorNo: number, stackNo: number) {
  ...
  allStackInwards.forEach(inward => {
    ...
    if (stackAllocated > 0) {
      ...
      transactions.push({
        ...
        type: 'INWARD',
        receiptNo: inward.receiptNumber || inward.weighbridgeSlipNo || (inward as any).receiptNo || `INW-${inwardIdStr.slice(-6).toUpperCase()}`,
  ...
  outwardsDocs.forEach((outward: any) => {
    ...
    transactions.push({
      ...
      type: 'OUTWARD',
      receiptNo: outward.receiptNumber || outward.weighbridgeSlipNo || `OUT-${outward._id.toString().slice(-6).toUpperCase()}`,
      commodity: commodityDisplay,
